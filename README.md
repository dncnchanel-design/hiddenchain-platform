...
《隐链明算——基于 Agent 原生可信数据空间的电力交易新范式》的可运行 MVP 单仓库。

## 已实现的核心闭环

`用户请求 → Agent四场景编排 → DID/VC校验 → 数据合同授权 → 隐私策略路由 → VPP偏差响应 → 调度安全校核 → PSI/MPC结算 → 多方签名 → 三阶段存证 → 监管审计报告`

平台覆盖五类账号、五大可信能力、六类专业 Agent、四场景耦合主流程、用户用电隐私分析和原 17 个界面模块。新能源消纳、市场交易、虚拟电厂运营与电网调度共用同一交易批次、数据许可、计算计划和证据索引。真实 EDC、SecretFlow、FISCO BCOS、WeIdentity 与大模型均通过适配器边界预留；默认实现不会伪装成生产能力，而是用模拟节点和假数据稳定演示完整流程。
...
进入“Agent 协同”页面后，可选择可信交易胶囊并点击“依次真实调用六个 Agent”，也可以在各 Agent 卡片中修改指令后逐个调用。真实调用成功时页面会显示 DeepSeek 模型、请求 ID、耗时和 Token 用量；没有这些凭证时不得视为真实 AI 调用成功。
...
- 四场景耦合：新能源预测摘要进入市场任务，交易偏差先由虚拟电厂资源池调节，剩余偏差必须通过调度安全闸门。
...
- 电力业务：新能源预测资产、虚拟电厂资源池、调度安全边界、偏差响应和自动结算安全闸门
...
登录认证、平台总览、角色工作台、发电侧数据、售电与用电数据、可信数据空间、规则与智能合约、结算任务、隐私计算、结算结果、区块链存证、监管审计、Agent 协同、异常处置、全过程日志、主体与 DID、可信报告、运行指标。
...
- “可信数据空间”前端页面展示目录、协议能力、协商状态、使用次数和回执记录。
...
启动后使用 `exchange / exchange123` 登录，并按“结算任务 → 隐私计算 → 结算结果 → 区块链存证 → 监管审计 → 可信报告”检查主链路。
