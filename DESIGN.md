...
colors:
  ...
  grid-green-800: "#006151"
  grid-green-700: "#007462"
  grid-green-600: "#0a806c"
  grid-green-500: "#0a806c"
  grid-green-400: "#61ac9a"
  grid-green-300: "#91c8b9"
  grid-green-200: "#bdded5"
  grid-green-100: "#def0eb"
  grid-green-50: "#effaf6"
...
components:
  system-bar:
    backgroundColor: "{colors.system-bar}"
...
整体采用翡翠绿系统栏、白色横向业务栏目、冷灰工作面、`#0A806C` 明亮电网绿品牌识别、1px 结构线和小圆角。系统拒绝通用 SaaS/AI 后台、紫色或蓝紫科技感、玻璃质感、营销 Hero、横幅式欢迎区、超大数据展示，以及对旧式门户的表面仿古。

**Key Characteristics:**

- 48px 翡翠绿系统栏、42px 白色一级导航、34px 面包屑栏构成稳定的 124px 顶部上下文。
- `#0A806C` 品牌绿只承担当前项、主操作、链接、焦点和少量结构提示；大面积内容保持中性。
...
颜色体系是“翡翠绿系统上下文 + 冷灰生产工作面 + 克制电网深绿 + 明确语义色”。前置 YAML 是默认规范值，实现由 `ProductBrandConfig.brandTheme.primary` 经统一色阶算法注入 `frontend/src/styles.css` 中的语义 Token。系统栏颜色独立于可白标品牌基准，不声明为任何客户的官方品牌色。

### Primary

- **完整原始色阶**（`grid-green-50` 至 `grid-green-950`）：默认品牌基准与主操作统一为 `#0A806C`，仅供主题生成器使用。
...
- **焦点与反白层**（`brand-primary-focus` / `brand-text-on-primary`）：焦点环统一为 `rgba(10, 128, 108, 0.18)`，主色表面文字统一为白色。

### Neutral

- **系统翡翠绿**（`system-bar`）：独立于品牌主题，仅用于顶部系统身份与运行上下文，不覆盖业务内容区；当前颜色为 `#0B7768`，白字对比度约为 `5.45:1`。
...
应用壳层没有常驻左侧栏，也没有工作空间切换器。顶部依次是 48px 翡翠绿系统栏、42px 白色一级导航和 34px 冷灰面包屑栏；其下直接进入页面标题、操作与高密度业务内容。系统栏承载品牌、机构、角色、环境、风险入口和用户菜单；一级导航表达业务域，面包屑保持 1–3 层任务位置。
