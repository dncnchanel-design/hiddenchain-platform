...
while ((Get-Date) -lt $endsAt) {
    ...
    try {
        ...
        $cycle.Add((Invoke-Probe "api.evidence" "$BaseUrl/api/chain/evidence?task_id=task-ready-t01" -RequestHeaders $authHeaders))
        $cycle.Add((Invoke-Probe "api.timeline" "$BaseUrl/api/audit/timeline/task-ready-t01" -RequestHeaders $authHeaders))
        ...
        $cycle.Add((Invoke-Probe "api.catalog" "$BaseUrl/api/data/catalog?trade_batch_no=TB-2026-07-T01" -RequestHeaders $authHeaders))
        $cycle.Add((Invoke-Probe "api.agreements" "$BaseUrl/api/data/agreements?trade_batch_no=TB-2026-07-T01" -RequestHeaders $authHeaders))
        ...
        $cycle.Add((Invoke-Probe "api.agent.events" "$BaseUrl/api/agents/events?task_id=task-ready-t01" -RequestHeaders $authHeaders))
