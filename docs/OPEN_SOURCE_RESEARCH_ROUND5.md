...
6. EDC、SecretFlow、walt.id、immudb 等重型能力仍必须经过真实部署、许可证、密钥管理和故障演练后才能进入生产边界；它们不能绕过 OPA、结果审计或人工确认。

## 本轮结论

当前最稳妥的路线仍然是“基于现有系统改造”：用 OpenDP 补齐差分隐私的可执行实现，用 OpenLineage 补齐标准化血缘，用 OpenTelemetry 补齐运行观测，用 OSV-Scanner + Syft 把依赖漏洞和 SBOM 纳入 GitHub 流程；EDC/SecretFlow/真实 DID 服务保留为适配器替换路线，不因追求开源数量而扩大部署和审计风险。
