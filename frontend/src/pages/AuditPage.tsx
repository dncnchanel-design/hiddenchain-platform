...
import { AGENT_LABELS, EVIDENCE_TYPE_LABELS, MESSAGE_TYPE_LABELS, STAGE_LABELS, labelForCode, type JsonRecord } from "../types";
...
function eventTitle(event: JsonRecord) {
  ...
  if (event.kind === "EVIDENCE_RECORD") return `${STAGE_LABELS[title[0]] || labelForCode(title[0], "证据阶段")} · ${EVIDENCE_TYPE_LABELS[title[1]] || labelForCode(title[1], "证据记录")}`;
  if (event.kind === "AGENT_EVENT") return `${AGENT_LABELS[title[0]] || labelForCode(title[0], "能力服务")} · ${MESSAGE_TYPE_LABELS[title[1]] || labelForCode(title[1], "事件类型")}`;
...
export function AuditPage() {
  ...
  return (
    <>
      ...
      <FilterBar>
        <label><span>审计对象</span><select value={effectiveTaskId} disabled={Boolean(linkedTaskId)} onChange={(event) => { setTaskId(event.target.value); setAnswer(null); }}><option value="">请选择</option>{tasks.data.map((item) => <option key={item.task_id} value={item.task_id}>{labelForCode(item.capsule_id, "已登记任务")} · {item.task_name}</option>)}</select></label>
        ...
        <Surface title="证据辅助解释">
          <div className="structured-audit-check">
            ...
            {answer && <div className="audit-check-result"><header><strong>辅助解释</strong><StatusTag value={answer.fallback ? "MEDIUM" : "INFO"} label={answer.fallback ? "模板降级" : "生成说明"} /></header><div className="audit-answer-meta"><div><span>置信度</span><strong>{confidenceLabels[String(answer.confidence || "")] || "未标注"}</strong></div><div><span>证据引用</span><strong>{answer.grounded ? "已关联" : "未关联"}</strong></div><div><span>生成方式</span><strong>{answer.fallback ? "结构化模板" : "生成式能力"}</strong></div></div><p>{answer.answer || "—"}</p>{answer.boundary && <div className="audit-answer-boundary"><strong>能力边界</strong><span>{String(answer.boundary)}</span></div>}<DataTable keyField="evidence_id" rows={answer.citations || []} empty="未引用具体凭证" label="辅助解释引用凭证" columns={[
              { key: "stage", label: "阶段", render: (row) => STAGE_LABELS[row.stage] || labelForCode(row.stage, "已登记阶段") },
