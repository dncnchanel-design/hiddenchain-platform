...
export function AuditPage() {
  ...
  return (
    <>
      <PageHeader eyebrow="安全审计" title="业务验证审计" description="按时间查看授权、计算、结果和凭证。" actions={<Button icon={RefreshCw} onClick={() => { void tasks.reload(); void timeline.reload(); }}>刷新</Button>} />
      ...
      <div className="audit-layout">
        <Surface title="处理时间线" note={`${timeline.data?.events?.length || 0} 条记录`}>
        ...
        <Surface title="审计问答">
          <div className="agent-query">
            <div className="agent-identity"><Bot size={23} /><div><strong>审计助手</strong><span>已连接</span></div><StatusTag value="VALID" label="可用" /></div>
