...
import { ACTION_LABELS, TARGET_TYPE_LABELS, labelForCode, type JsonRecord } from "../types";
  ...
  return (
    <>
      ...
      <FilterBar actions={<span className="filter-result-count">显示 {rows.length} / {data.length} 条</span>}>
        ...
        <label><span>操作动作</span><select value={action} onChange={(event) => setAction(event.target.value)}><option value="">全部动作</option>{actions.map((item) => <option key={item} value={item}>{ACTION_LABELS[item] || labelForCode(item, "已登记动作")}</option>)}</select></label>
      ...
      <Surface title="记录列表" meta={`${rows.length} 条`}>
        <DataTable
          ...
          columns={[
            ...
            { key: "action_code", label: "动作", minWidth: 190, render: (row) => ACTION_LABELS[row.action_code] || labelForCode(row.action_code, "已登记动作") },
            { key: "target_type", label: "对象类型", minWidth: 120, render: (row) => TARGET_TYPE_LABELS[row.target_type] || labelForCode(row.target_type, "已登记对象") },
      ...
      {selected && <DetailDrawer title="日志详情" onClose={() => setSelected(null)} footer={<Button onClick={() => setSelected(null)}>关闭</Button>}>
        <div className="detail-grid"><div><span>日志编号</span><IdText value={selected.log_id} /></div><div><span>追踪编号</span><IdText value={selected.trace_id} /></div><div><span>动作</span><strong>{ACTION_LABELS[selected.action_code] || labelForCode(selected.action_code, "已登记动作")}</strong></div><div><span>操作结果</span><StatusTag value={selected.result} /></div><div><span>操作主体</span><strong>{selected.actor_name || "—"}</strong></div><div><span>发生时间</span><DateTimeText value={selected.occurred_at} /></div><div><span>对象类型</span><strong>{TARGET_TYPE_LABELS[selected.target_type] || labelForCode(selected.target_type, "已登记对象")}</strong></div><div><span>对象编号</span><IdText value={selected.target_id} /></div></div>
