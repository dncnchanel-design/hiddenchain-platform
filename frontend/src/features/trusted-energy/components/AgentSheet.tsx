...
export function AgentSheet({ open, onOpenChange }: { open: boolean; onOpenChange: (open: boolean) => void }) {
        ...
        if (isActive()) {
          const diagnostic = reason instanceof ApiError
            ...
            : reason instanceof Error
              ? `Agent 会话加载失败（${reason.name}: ${reason.message.slice(0, 120)}），请重试。`
              : "Agent 会话加载失败，请重试。";
          setError(diagnostic);
        }
      } finally {
  ...
  async function send(content = message) {
    ...
    try {
      ...
      setError(reason instanceof ApiError ? reason.message : "Agent 指令未记录，请重试。");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function runPlanAction(plan: AssistantPlan, action: "execute" | "cancel" | "retry") {
    ...
    try {
      ...
      setError(reason instanceof ApiError ? reason.message : "Agent 计划动作失败，请刷新后重试。");
    } finally {
      setBusy(false);
    }
  }
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}>
    <DialogContent className="energy-agent-sheet">
      <div className="energy-agent-heading"><div className="energy-agent-title"><span className="energy-agent-icon"><Bot size={17} /></span><div><DialogTitle>隐链智能助手</DialogTitle><DialogDescription>确定性本地规划器；写动作只进入人工复核</DialogDescription></div></div><IconButton label="关闭 Agent 助手" onClick={() => onOpenChange(false)}><X size={16} /></IconButton></div>
      <div className="energy-agent-body">
        ...
        {!loading && !error && <>
          <div className="energy-agent-message energy-agent-message-bot"><span className="energy-agent-avatar"><Bot size={14} /></span><div><p>我只会调用后端 allowlist 中的真实查询或生成待复核计划，不会伪造执行结果。</p><small>会话主体：<code>{session?.entity_id || "当前登录主体"}</code></small></div></div>
          ...
          {messages.filter((item) => item.role === "ASSISTANT").slice(-3).map((item) => <div className="energy-agent-message energy-agent-message-bot" key={item.message_id}><span className="energy-agent-avatar"><Bot size={14} /></span><div><p>{item.content}</p><small>{item.intent_code || "ASSISTANT"} · {item.capability_state || item.status}</small></div></div>)}
          {latestPlan && <section className="energy-agent-plan"><div className="energy-agent-section-heading"><div><span className="energy-section-kicker">LOCAL PLAN</span><h3>真实计划 · {latestPlan.intent_code}</h3></div><Badge tone={capabilityTone(latestPlan.capability_state)} dot>{latestPlan.status}</Badge></div><ol>{latestPlan.steps.map((step) => <li key={step.step_id}><span className={step.status === "SUCCEEDED" ? "energy-plan-done" : ""}>{step.status === "SUCCEEDED" ? <Check size={13} /> : step.sequence_no}</span><span>{step.action_code} · {step.status}{step.request_id ? ` · ${step.request_id}` : ""}{step.output && Object.keys(step.output).length ? <small>{JSON.stringify(step.output)}</small> : null}</span></li>)}</ol><div className="energy-agent-plan-note"><ShieldCheck size={14} /><span>来源：<code>{latestPlan.source_of_truth || "assistant_planner"}</code> · 能力：<code>{latestPlan.capability_state || "未登记"}</code></span></div><div className="trusted-submit-actions">{latestPlan.allowed_actions?.includes("execute") && <Button variant="primary" size="sm" busy={busy} onClick={() => void runPlanAction(latestPlan, "execute")}>执行计划</Button>}{latestPlan.allowed_actions?.includes("cancel") && <Button variant="secondary" size="sm" disabled={busy} onClick={() => void runPlanAction(latestPlan, "cancel")}>取消计划</Button>}{latestPlan.allowed_actions?.includes("retry") && <Button variant="secondary" size="sm" disabled={busy} onClick={() => void runPlanAction(latestPlan, "retry")}>重试计划</Button>}</div></section>}
          ...
          <section className="energy-agent-shortcuts"><h3>可用工具</h3><div>{tools.map((tool) => <span className="trusted-muted" key={tool.tool_code}>{tool.tool_name || tool.tool_code}</span>)}{!tools.length && <span className="trusted-muted">当前角色暂无登记工具</span>}</div></section>
        </>}
      </div>
      <div className="energy-agent-composer"><Input value={message} onChange={(event) => setMessage(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void send(); }} placeholder="输入受支持的本地查询或需求…" aria-label="输入 Agent 指令" disabled={loading || Boolean(error) || !session} /><Button variant="primary" size="icon" busy={busy} disabled={!message.trim() || !session} onClick={() => void send()} aria-label="发送"><Send size={15} /></Button></div>
      <footer className="energy-agent-footer"><span><span className="energy-status-dot" />{session?.capability_state || "受控环境"}</span><span>{session?.state_version ? `会话 V${session.state_version}` : "会话未建立"}</span><CornerDownLeft size={13} /></footer>
    </DialogContent>
  </Dialog>;
}
