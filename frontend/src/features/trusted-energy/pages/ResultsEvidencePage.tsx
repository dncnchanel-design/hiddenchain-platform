...
import { RESULT_SCOPE_LABELS, STAGE_LABELS, TARGET_TYPE_LABELS, labelForCode } from "../../../types";
...
function resultStatus(value: string) {
  return ({ CONFIRMED: "已确认", UNCONFIRMED: "待确认", PENDING: "待确认", REJECTED: "已拒绝" } as Record<string, string>)[value] || labelForCode(value, "未登记状态");
}
...
export function ResultsEvidencePage() {
  ...
  const metrics = useMemo(() => {
    ...
    return resultRows(detail.result.result).slice(0, 4).map(([label, value], index) => ({
      label,
      value: scalar(value),
      detail: RESULT_SCOPE_LABELS[detail.result.result_scope || ""] || labelForCode(detail.result.result_scope, "后端结果摘要"),
  ...
  async function verify(evidenceId: string) {
    ...
    try {
      ...
      setVerifyState((current) => ({ ...current, [evidenceId]: payload.matched ? (payload.external_receipt_verified ? "哈希匹配 · 外部回执已核验" : "哈希匹配 · 本地演示锚定") : "核验未通过" }));
    } catch (error) {
  ...
  return <PageFrame title="计算结果与存证" description={detail ? `结果 ${detail.result.result_id} 的摘要、签名和证据记录来自真实后端。` : "读取真实结果与存证记录。"} back={routeForView("results")} action={<><>{detail && <StatusBadge value={resultStatus(detail.result.confirm_status)} />}</><Button variant="secondary" onClick={detailRemote.reload} busy={detailRemote.refreshing}><RefreshCw size={14} />刷新</Button></>}>
    ...
    {detail && <>
      ...
      <div className="trusted-result-grid"><Card><CardHeader><SurfaceHeader title="结果摘要" description="只展示后端结果记录中的真实字段" action={<BadgeCheck size={16} />} /></CardHeader><CardContent className="trusted-table-wrap"><Table><TableHeader><TableRow><TableHead>指标</TableHead><TableHead>结果</TableHead><TableHead>来源</TableHead></TableRow></TableHeader><TableBody>{resultRows(detail.result.result).map(([label, value]) => <TableRow key={label}><TableCell>{label}</TableCell><TableCell><strong className="trusted-result-number">{scalar(value)}</strong></TableCell><TableCell><Badge tone="info">后端结果</Badge></TableCell></TableRow>)}{!resultRows(detail.result.result).length && <TableRow><TableCell colSpan={3}>后端未登记结构化结果摘要</TableCell></TableRow>}</TableBody></Table></CardContent></Card><Card><CardHeader><SurfaceHeader title="可信验证" description="真实结果哈希、签名与输入关联" action={<Hash size={16} />} /></CardHeader><CardContent><div className="trusted-hash-list"><div><span>结果哈希</span><code>{shortHash(detail.result.result_hash)}</code><Button variant="link" size="sm" disabled={!detail.result.result_hash} aria-label="复制结果哈希" onClick={() => copyValue("结果哈希", detail.result.result_hash || "—")}><Copy size={13} />复制</Button><Badge tone={detail.result.result_hash ? "success" : "warning"}>{detail.result.result_hash ? "已登记" : "缺失"}</Badge></div><div><span>任务编号</span><code>{detail.result.task_id}</code><Button variant="link" size="sm" aria-label="复制任务编号" onClick={() => copyValue("任务编号", detail.result.task_id)}><Copy size={13} />复制</Button></div><div><span>确认状态</span><StatusBadge value={resultStatus(detail.result.confirm_status)} /></div><div><span>状态版本</span><code>V{taskVersion}</code></div></div><div className="trusted-divider-spacer" /><div className="trusted-signature-list">{detail.signatures.map((signature) => <div key={signature.signature_id}><span><strong>{TARGET_TYPE_LABELS[signature.target_type || ""] || labelForCode(signature.target_type, "签名对象")}</strong><small>{signature.signer_did || signature.signer_org_id || "未登记签署主体"}</small></span><code>{shortHash(signature.target_hash)}</code><StatusBadge value={signature.verify_status || "未验证"} /></div>)}{!detail.signatures.length && <span className="trusted-muted">暂无签名记录</span>}</div></CardContent></Card></div>
      <Card><CardHeader><SurfaceHeader title="证据与链路" description="只在外部交易回执核验成功后显示外部发布；本地模式明确标记为演示锚定" action={<Blocks size={16} />} /></CardHeader><CardContent><div className="trusted-chain-record">{detail.evidence.map((evidence) => <div key={evidence.evidence_id}><div><small>{STAGE_LABELS[evidence.stage || ""] || labelForCode(evidence.stage, "证据")}</small><strong><code>{evidence.evidence_id}</code></strong></div><div><small>证据哈希</small><code>{shortHash(evidence.evidence_hash)}</code></div><div><small>锚定后端</small><span>{labelForCode(evidence.chain_code, "未登记")}</span></div><div><small>链状态</small>{evidence.tx_hash ? <code>{shortHash(evidence.tx_hash)}</code> : <Badge tone="warning">待锚定</Badge>}</div>{evidence.block_height !== null && evidence.block_height !== undefined && evidence.tx_hash && <div><small>区块高度</small><strong>{evidence.block_height}</strong></div>}<StatusBadge value={evidence.status || "未登记"} /><Button variant="secondary" size="sm" busy={verifyingId === evidence.evidence_id} disabled={Boolean(verifyingId)} onClick={() => verify(evidence.evidence_id)}><ShieldCheck size={14} />核验证据</Button>{verifyState[evidence.evidence_id] && <span className="trusted-muted" role="status">{verifyState[evidence.evidence_id]}</span>}</div>)}{!detail.evidence.length && <RemoteState empty emptyLabel="暂无单条链证据记录" />}</div><div className="trusted-divider-spacer" /><div className="trusted-chain-record">{detail.formal_evidence.map((batch) => <div key={batch.batch_id}><div><small>证据批次</small><strong><code>{batch.batch_id}</code></strong></div><div><small>默克尔树根</small><code>{shortHash(batch.merkle_root)}</code></div><div><small>待发送证据</small><span>{batch.outbox.length} 条 · {labelForCode(batch.status, "未登记")}</span></div><div><small>链上锚定</small><span>{batch.anchors.length ? batch.anchors.map((anchor) => `${anchor.transaction_hash ? shortHash(anchor.transaction_hash) : "待锚定"}${anchor.external_receipt_verified ? " · 外部回执已核验" : " · 本地演示/未配置"}`).join("、") : "未登记"}</span></div></div>)}{!detail.formal_evidence.length && <span className="trusted-muted">暂无正式证据批次</span>}</div></CardContent></Card>
      <Card><CardHeader><SurfaceHeader title="结果结论" description="只有后端返回结果确认能力才能提交；动作经过当前状态版本校验与幂等保护" action={<Check size={16} />} /></CardHeader><CardContent><div className="trusted-submit-actions"><Button variant={decision === "APPROVE" ? "primary" : "secondary"} disabled={!allowedActions.includes("confirm_result") || busy} onClick={() => setDecision("APPROVE")}>确认通过</Button><Button variant={decision === "REJECT" ? "danger" : "secondary"} disabled={!allowedActions.includes("confirm_result") || busy} onClick={() => setDecision("REJECT")}><X size={14} />提出异议</Button><span className="trusted-muted">{allowedActions.includes("confirm_result") ? `当前版本 V${taskVersion}` : "当前角色或任务状态不可提交结论"}</span></div><Textarea value={opinion} onChange={(event) => setOpinion(event.target.value)} placeholder="填写结果结论意见" aria-label="结果结论意见" disabled={!allowedActions.includes("confirm_result")} />{actionError && <p className="trusted-muted" role="alert">{actionError}</p>}<div className="trusted-submit-actions"><Button variant="primary" busy={busy} disabled={!allowedActions.includes("confirm_result") || !opinion.trim()} onClick={submitDecision}>提交{decision === "APPROVE" ? "确认" : "异议"}</Button><span className="trusted-muted">结果确认会产生真实签名与审计副作用；不可撤回。</span></div></CardContent></Card>
    </>}
  </PageFrame>;
}
