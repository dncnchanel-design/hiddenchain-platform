...
const regulatoryPurposeIds = new Set<string>(regulatoryUseCases.map((item) => item.id));

function isRegulatoryPurpose(value: string): value is typeof regulatoryUseCases[number]["id"] {
  return regulatoryPurposeIds.has(value);
}
...
export function ApplyPage() {
  ...
  const [purpose, setPurpose] = useState<PurposeId>(useCases[0].id);
  ...
  const selectedRegulatoryUseCase = regulatoryUseCases.find((item) => item.id === purpose);
  const selectedUseCase = [...useCases, ...regulatoryUseCases].find((item) => item.id === purpose);
  ...
  useEffect(() => {
    if (isRegulator && !regulatoryPurpose) {
      setPurpose(regulatoryUseCases[0].id);
      setDurationDays(regulatoryUseCases[0].maxDurationDays);
    }
  }, [isRegulator, regulatoryPurpose]);
  ...
  async function submitRequest() {
    ...
    const requestedFields = purpose === "MODEL_TRAINING" ? ["model_metrics"] : purpose === "AUDIT_REVIEW" ? ["audit_summary", "evidence_refs"] : ["summary", "quality_metrics"];
    const algorithmCode = `CONTROLLED_${purpose}_V1`;
    const fingerprint = JSON.stringify({ asset: asset.asset_id, version: currentVersion?.version_id || null, purpose, method, authorityRef, duration: effectiveDurationDays });
    ...
    try {
      const result = await createUsageRequest({
        ...
        purpose,
        usage_mode: method,
        requested_scope: { purpose_code: purpose, algorithm_code: algorithmCode, output_mode: outputMode, raw_data_export: false },
    ...
    {payload && asset && <>
      ...
      {submission ? <Card className="trusted-submit-success"><CardContent><span className="trusted-success-icon"><Check size={24} /></span><h2>申请已提交至提供方审核</h2><p>申请编号 <code>{submission.request_id}</code> 已生成，当前状态为 <strong>{requestStatusLabel(submission.status)}</strong>，状态版本 {submission.state_version}。</p><p>申请期限 {submission.duration_days} 日 · {policySourceLabel(submission.duration_policy?.source)}。系统不会代替提供方批准，也不会自动执行计算。</p><div className="trusted-submit-actions"><Button variant="secondary" onClick={() => navigate(routeForView("catalog"))}>返回数据目录</Button><Button variant="primary" onClick={() => navigate(`${routeForView("authorizations")}?view=outbound&request=${encodeURIComponent(submission.request_id)}`)}>查看我的申请 <ArrowRight size={14} /></Button></div></CardContent></Card> : <Card className="trusted-step-card"><CardHeader><CardTitle>{steps[step]}</CardTitle><p>{step === 0 ? "请选择本次申请的业务目标，目的将写入后续授权记录。" : step === 1 ? "选择提供方允许的处理方式，最终以提供方审核为准。" : step === 2 ? "确认数据范围、结果口径和责任边界。" : "提交前核对真实资产版本与本地受控能力边界。"}</p></CardHeader><CardContent>
        {step === 0 && <>
          ...
          <div className="trusted-option-grid">{useCases.map(({ id, title, desc, icon: Icon }) => <button type="button" className={`trusted-option-card${purpose === id ? " is-selected" : ""}`} key={id} onClick={() => selectPurpose(id)}><span className="trusted-option-radio">{purpose === id && <i />}</span><Icon size={18} /><span><strong>{title}</strong><small>{desc}</small></span></button>)}</div>
          {isRegulator && <><p className="trusted-muted">监管白名单用途</p><div className="trusted-option-grid">{regulatoryUseCases.map(({ id, title, desc, icon: Icon }) => <button type="button" className={`trusted-option-card${purpose === id ? " is-selected" : ""}`} key={id} onClick={() => selectPurpose(id)}><span className="trusted-option-radio">{purpose === id && <i />}</span><Icon size={18} /><span><strong>{title}</strong><small>{desc}</small></span></button>)}</div></>}
        </>}
