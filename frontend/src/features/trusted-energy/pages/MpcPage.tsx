...
import { ArrowRight, CheckCircle2, Cpu, Link2, Network, Plus, Radio, RefreshCw, ShieldCheck, TerminalSquare } from "lucide-react";
...
export function MpcPage() {
  ...
  const canStartQuery = Boolean(session?.user.permissions?.includes("CREATE_COMPUTE_TASK"));
  ...
  return <PageFrame title="隐私计算" description={detail ? `查看真实计算任务 ${detail.job.job_id} 的参与方、固定算法、输出边界和回执。` : "查看授权后的固定算法计算任务与能力边界。"} back={detail ? (detail.job.task_kind === "TRUSTED_QUERY" ? routeForView("query") : routeForView("ttc", detail.job.task_id)) : routeForView("workbench")} action={detail ? <><Badge tone={capabilityTone(detail.external_execution.capability_state)} dot>{capabilityLabel(detail.external_execution.capability_state)}</Badge>{canStartQuery && <Button variant="primary" onClick={() => navigate(routeForView("query"))}><Plus size={14} />发起智能计算</Button>}</> : <><Button variant="secondary" onClick={listRemote.reload} busy={listRemote.refreshing}><RefreshCw size={14} />刷新</Button>{canStartQuery && <Button variant="primary" onClick={() => navigate(routeForView("query"))}><Plus size={14} />发起智能计算</Button>}</>}>
