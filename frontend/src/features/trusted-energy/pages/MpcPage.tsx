...
export function MpcPage() {
  ...
  return <PageFrame title="多方安全计算任务" description={detail ? `查看真实计算任务 ${detail.job.job_id} 的参与方、进度、日志和回执。` : "读取真实隐私计算任务与能力边界。"} back={detail ? routeForView("ttc", detail.job.task_id) : routeForView("workbench")} action={detail ? <Badge tone={capabilityTone(detail.external_execution.capability_state)} dot>{capabilityLabel(detail.external_execution.capability_state)}</Badge> : <Button variant="secondary" onClick={() => void listRemote.reload()} busy={listRemote.refreshing}><RefreshCw size={14} />刷新</Button>}>
    ...
    {detail && <>
      ...
      <Card><CardHeader><SurfaceHeader title="计算控制" description="仅在后端明确开放可用动作时启用写动作" /></CardHeader><CardContent><div className="trusted-submit-actions"><Button variant="secondary" busy={controlBusy === "retry"} disabled={!detail.allowed_actions?.includes("retry") || Boolean(controlBusy)} title={detail.allowed_actions?.includes("retry") ? "执行后端重试动作" : detail.action_reasons?.retry || "后端未返回重试能力"} onClick={() => void runControl("retry")}>重试计算</Button><Button variant="danger" busy={controlBusy === "cancel"} disabled={!detail.allowed_actions?.includes("cancel") || Boolean(controlBusy)} title={detail.allowed_actions?.includes("cancel") ? "执行后端取消动作" : detail.action_reasons?.cancel || "后端未返回取消能力"} onClick={() => void runControl("cancel")}>取消计算</Button><span className="trusted-muted">{controlError || detail.action_reasons?.retry || detail.action_reasons?.cancel || "当前没有可执行控制动作"}</span><span className="trusted-muted">可信执行环境：{detail.external_execution.tee_attestation ? "已提供远程证明" : "未配置"} · 跨域参与方：{detail.external_execution.cross_domain_participants.length}</span></div></CardContent></Card>
    </>}
  </PageFrame>;
}
