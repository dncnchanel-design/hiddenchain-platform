...
function recordTitle(record: AuditRecord) {
  if (record.record_type === "AUDIT_REPORT") return String(record.details?.title || "审计报告");
  return record.action_code || record.target_type || "审计记录";
}

function transitionNodes(detail: AuditTaskPayload) {
  return detail.transitions.map((item, index) => {
    const state = String(item.to_state || item.state || "未知");
    return { id: String(item.transition_id || `${state}-${index}`), label: state, detail: String(item.reason || item.trigger_code || "真实状态转移"), time: formatDate(typeof item.occurred_at === "string" ? item.occurred_at : null), done: true };
  });
}
...
export function AuditCenterPage() {
  ...
  if (!taskId) {
    return <PageFrame title="审计中心" description="以真实任务为主线复核审计日志、状态转移、报告与证据。" action={<div className="trusted-submit-actions"><Button variant="secondary" busy={exporting === "json"} onClick={() => void exportRecords("json")}><Download size={14} />JSON 导出</Button><Button variant="secondary" busy={exporting === "csv"} onClick={() => void exportRecords("csv")}><Download size={14} />CSV 导出</Button><Button variant="secondary" onClick={() => void listRemote.reload()} busy={listRemote.refreshing}><RefreshCw size={14} />刷新</Button></div>}>
      ...
      {list && list.items.length > 0 && <Card className="trusted-audit-list"><CardHeader><SurfaceHeader title="审计记录" description="服务端按组织与角色范围返回日志和审计报告" action={<Badge tone="info">第 {list.page} 页 · 共 {list.total} 条</Badge>} /></CardHeader><CardContent><div className="trusted-audit-items">{list.items.map((record) => { const targetTask = taskIdFor(record); return <button className="trusted-audit-item" type="button" key={`${record.record_type}-${record.record_id}`} disabled={!targetTask} onClick={() => targetTask && navigate(routeForView("audit", targetTask))}><div><strong><code>{record.record_id}</code></strong><StatusBadge value={record.result || "已记录"} /></div><span>{recordTitle(record)}</span><small>{formatDate(record.occurred_at)} · {record.target_type || "—"}</small>{targetTask && <small><code>任务 {targetTask}</code></small>}</button>; })}</div><div className="trusted-submit-actions"><Button variant="link" size="sm" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>上一页</Button><Button variant="link" size="sm" disabled={page * list.page_size >= list.total} onClick={() => setPage((value) => value + 1)}>查看更多任务</Button></div></CardContent></Card>}
    </PageFrame>;
  }

  return <PageFrame title="审计中心" description={detail ? `任务 ${detail.task.task_id} 的完整审计链、报告与证据来自真实后端。` : "读取真实任务审计详情。"} back={routeForView("audit")} action={<Button variant="secondary" onClick={() => void detailRemote.reload()} busy={detailRemote.refreshing}><RefreshCw size={14} />刷新</Button>}>
