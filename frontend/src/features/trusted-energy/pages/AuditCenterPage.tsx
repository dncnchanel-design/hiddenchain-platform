...
export function AuditCenterPage() {
  ...
  if (!taskId) {
    return <PageFrame title="审计中心" description="以真实任务为主线复核审计日志、状态转移、报告与证据。" action={<div className="trusted-submit-actions"><Button variant="secondary" busy={exporting === "json"} onClick={() => void exportRecords("json")}><Download size={14} />结构化数据导出</Button><Button variant="secondary" busy={exporting === "csv"} onClick={() => void exportRecords("csv")}><Download size={14} />表格数据导出</Button><Button variant="secondary" onClick={() => void listRemote.reload()} busy={listRemote.refreshing}><RefreshCw size={14} />刷新</Button></div>}>
