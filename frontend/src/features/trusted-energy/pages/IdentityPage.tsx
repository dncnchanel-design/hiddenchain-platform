  ...
  return <PageFrame title="身份中心" description="管理当前主体的去中心化身份标识、数字证书与数据空间连接器边界。" action={<Button variant="secondary" onClick={() => void remote.reload()} busy={remote.refreshing}><RefreshCw size={14} />刷新身份状态</Button>}>
    ...
    {identity && <>
      <div className="trusted-identity-layout">
        <nav className="trusted-identity-nav" aria-label="身份中心分区">
          <a className="is-active" href="#trusted-identity-subject"><UserRound size={14} />身份概览</a>
          <a href="#trusted-identity-did"><Fingerprint size={14} />DID信息</a>
          ...
          <a href="#trusted-identity-connector"><Link2 size={14} />Connector绑定</a>
        </nav>
        <div className="trusted-identity-content">
          <div className="trusted-detail-grid trusted-identity-grid">
            ...
            <div className="trusted-detail-side"><Card id="trusted-identity-did"><CardHeader><SurfaceHeader title="DID信息" description={`来源：${labelForCode(identity.did.source_of_truth, "去中心化身份记录")}`} action={<Fingerprint size={17} />} /></CardHeader><CardContent><div className="trusted-did-value"><code>{valueOrDash(identity.did.did_id)}</code><IconButton label="复制去中心化身份标识" onClick={() => void copyValue(identity.did.did_id)}><Copy size={13} /></IconButton></div><dl className="trusted-definition-list"><div><dt>公钥指纹</dt><dd><code>{valueOrDash(identity.did.public_key_fingerprint)}</code></dd></div><div><dt>链上地址</dt><dd><code>{valueOrDash(identity.did.chain_address)}</code></dd></div><div><dt>密钥托管</dt><dd><Badge tone="warning">本地受控环境</Badge></dd></div></dl><div className="trusted-note"><LockKeyhole size={14} /><span>私钥不离开当前受控环境；去中心化身份标识与凭证展示来自后端身份记录。</span></div></CardContent></Card><Card id="trusted-identity-connector"><CardHeader><SurfaceHeader title="数据空间 Connector" description={`来源：${labelForCode(identity.connector.source_of_truth, "数据源记录")}`} action={<Link2 size={17} />} /></CardHeader><CardContent><dl className="trusted-definition-list"><div><dt>连接器类型</dt><dd><code>{labelForCode(identity.connector.code, "数据空间连接器")}</code></dd></div><div><dt>协议版本</dt><dd>{identity.connector.protocol_version}</dd></div><div><dt>已登记数据源</dt><dd>{identity.connector.source_count}</dd></div><div><dt>当前能力</dt><dd><Badge tone="warning" dot>{labelForCode(identity.connector.capability_state, "适配器能力")}</Badge></dd></div><div><dt>连接状态</dt><dd><StatusBadge value={identity.connector.readiness} /></dd></div></dl><Button variant="secondary" disabled title="已阻断：当前后端未提供连接器管理动作"><Link2 size={14} />管理 Connector</Button></CardContent></Card></div>
          </div>
