...
function contractStatus(value: string) {
  return ({ ACTIVE: "已生效", NEGOTIATED: "协商中", PENDING: "待确认", REJECTED: "已拒绝", REVOKED: "已撤销", EXPIRED: "已过期" } as Record<string, string>)[value] || value || "未知";
}
...
function timelineFor(detail: ContractDetailPayload) {
  return detail.timeline.map((event, index) => ({
    ...
    detail: `${event.from_state} → ${event.to_state}`,
