...
function stateLabel(value: string) {
  return ({ INIT: "初始化", IDENTITY_VERIFY: "身份校验", DATA_AUTH: "数据授权", RULE_FROZEN: "规则冻结", COMPUTE_EXEC: "受控计算", RESULT_CONFIRM: "结果确认", EVIDENCE_ANCHOR: "证据锚定", ARCHIVED: "已归档", HUMAN_REVIEW: "人工复核", REWORK: "返工", INTERRUPTED: "已中断", CANCELLED: "已取消" } as Record<string, string>)[value] || value || "未知";
}
function statusLabel(value: string) {
  return ({ RUNNING: "执行中", PENDING: "待开始", SUCCEEDED: "已完成", ACTIVE: "已启用", COMPLETED: "已完成" } as Record<string, string>)[value] || value || "未知";
}
...
function eventTimeline(detail: TtcDetailPayload) {
  return detail.transitions.map((event, index) => ({
    ...
    detail: `${event.trigger_code} · ${event.reason || "无理由"}`,
...
function eventText(event: TtcEvent) {
  ...
  return `${event.kind} · ${event.state || "—"} · ${detailText}`;
}
...
export function TtcPage() {
  ...
  return <PageFrame title="TTC 任务详情" description={detail ? `沿状态机追踪真实任务 ${detail.task.task_id} 的主体、授权、受控计算和证据节点。` : "读取真实 TTC 任务与状态机记录。"} back={routeForView("workbench")} action={<>{detail && <StatusBadge value={statusLabel(detail.task.status)} />}<Button variant="secondary" disabled={!selectedTaskId} onClick={() => setLogsOpen(true)}><Logs size={14} />实时日志</Button></>}>
    ...
    {!routeTaskId && listPayload && <>
      ...
      {!listPayload.empty_state && <Card className="trusted-table-surface"><CardHeader><SurfaceHeader title="TTC 任务列表" description="先选择真实任务，再进入详情；页面不会自动替你选中第一条。" /></CardHeader><CardContent><div className="trusted-task-list">{listItems.map((task) => <button className="trusted-task-row" key={task.task_id} type="button" onClick={() => navigate(routeForView("ttc", task.task_id))}><span className="trusted-task-icon"><Activity size={15} /></span><span className="trusted-task-copy"><strong>{task.task_name}</strong><small><code>{task.task_id}</code> · {task.current_stage || stateLabel(task.ttc_state)}</small></span><span className="trusted-task-state"><StatusBadge value={statusLabel(task.status)} /><small>V{task.state_version}</small></span></button>)}</div><div className="trusted-step-footer" aria-label="TTC 任务分页"><span>第 {listPage} 页 · 共 {listPayload.total} 条</span><div><Button variant="secondary" size="sm" disabled={!canGoPrevious || listRemote.loading} onClick={() => updateListParam("page", String(listPage - 1))}>上一页</Button><Button variant="secondary" size="sm" disabled={!canGoNext || listRemote.loading} onClick={() => updateListParam("page", String(listPage + 1))}>下一页</Button></div></div></CardContent></Card>}
      {listRemote.refreshing && <div className="trusted-inline-status" role="status">正在刷新 TTC 列表…</div>}
    </>}
    ...
    {detail && <>
      <Card className="trusted-task-banner"><CardContent><div><small>任务 ID</small><strong><code>{detail.task.task_id}</code></strong></div><div><small>任务名称</small><strong>{detail.task.task_name}</strong></div><div><small>当前状态</small><strong>{stateLabel(detail.task.ttc_state)}</strong></div><div><small>当前尝试</small><strong>{detail.task.current_attempt ?? "—"}</strong></div><div><small>状态版本</small><strong><code>V{detail.task.state_version}</code></strong></div><div><small>能力来源</small><Badge tone="warning">{detail.source_of_truth || "settlement_tasks"}</Badge></div></CardContent></Card>
      <Card className="trusted-state-card"><CardHeader><SurfaceHeader title="状态轨迹" description="只展示后端持久化的 transition，不补造未发生节点" action={<Badge tone="info" dot>{stateLabel(detail.task.ttc_state)}</Badge>} /></CardHeader><CardContent>{timeline.length ? <div className="trusted-state-tracker">{timeline.map((event, index) => <div className={`trusted-state-node trusted-state-${event.state}`} key={event.id}><span className="trusted-state-marker">{event.state === "done" ? <Check size={13} /> : <Activity size={13} />}</span><strong>{event.label}</strong><small>{event.time}</small>{index < timeline.length - 1 && <i className="trusted-state-connector" />}</div>)}</div> : <RemoteState empty emptyLabel="暂无状态转换记录" />}</CardContent></Card>
      <div className="trusted-ttc-grid"><Card><CardHeader><SurfaceHeader title="节点事件" description="真实 transition 与时间戳" action={<FileClock size={16} />} /></CardHeader><CardContent>{timeline.length ? <Timeline events={timeline} /> : <RemoteState empty emptyLabel="暂无节点事件" />}</CardContent></Card><Card><CardHeader><SurfaceHeader title="任务属性" description="事实字段、规则冻结与执行快照" action={<ClipboardList size={16} />} /></CardHeader><CardContent><dl className="trusted-definition-list"><div><dt>任务类型</dt><dd>{detail.task.capsule_id || "—"}</dd></div><div><dt>当前阶段</dt><dd>{detail.task.current_stage || stateLabel(detail.task.ttc_state)}</dd></div><div><dt>执行快照</dt><dd><code>{detail.task.execution_snapshot_id || "未登记"}</code></dd></div><div><dt>快照哈希</dt><dd><code>{detail.task.execution_snapshot_hash || "—"}</code></dd></div><div><dt>尝试记录</dt><dd>{detail.attempts.length}</dd></div><div><dt>参与方</dt><dd>{detail.participants.length}</dd></div></dl><Progress value={detail.task.phase_progress_estimate?.value ?? 0} label={detail.task.phase_progress_estimate?.label || "阶段估算（非实时执行进度）"} /><small className="trusted-muted">来源：{detail.task.phase_progress_estimate?.source || "TTC_STATE_PHASE_ESTIMATE_V1"}</small></CardContent></Card></div>
      <Card><CardHeader><SurfaceHeader title="当前责任" description="系统动作仅在后端明确返回 allowed_actions 时启用" action={<ListChecks size={16} />} /></CardHeader><CardContent><div className="trusted-next-action"><span className="trusted-next-icon"><ShieldCheck size={17} /></span><div><strong>{stateLabel(detail.task.ttc_state)}</strong><p>{detail.transitions.at(-1)?.reason || "暂无最近状态理由；前端不会自行推进状态。"}</p></div><div className="trusted-submit-actions">{transitionActions.map((action) => <Button key={action} variant="primary" busy={transitionBusy} onClick={() => void runTransition(action.slice("transition:".length))}>推进至 {stateLabel(action.slice("transition:".length))}</Button>)}{!transitionActions.length && <Button variant="secondary" disabled title="后端当前只返回 view，状态推进需由系统服务端完成">状态推进不可用</Button>}</div></div>{transitionActions.length > 0 && <div className="trusted-option-grid"><Input value={transitionReason} onChange={(event) => setTransitionReason(event.target.value)} placeholder="状态转换理由" aria-label="状态转换理由" />{transitionError && <p className="trusted-muted" role="alert">{transitionError}</p>}</div>}</CardContent></Card>
    </>}
    <Dialog open={logsOpen} onOpenChange={setLogsOpen}><DialogContent className="energy-log-dialog"><DialogTitle>实时任务日志</DialogTitle><DialogDescription>打开后轮询真实 TTC events cursor；关闭即停止请求。</DialogDescription>{logLoading && !logItems.length && <RemoteState loading />}{logError && <RemoteState error={logError} onRetry={() => { setLogError(""); setLogRetryNonce((value) => value + 1); }} />}{!logError && !logLoading && !logItems.length && <RemoteState empty emptyLabel="暂无实时事件" />}<div className="trusted-log-stream">{logItems.map((event) => <div key={event.event_id}><time>{formatDate(event.occurred_at)}</time><code>{eventText(event)}</code></div>)}</div><Button variant="secondary" onClick={() => setLogsOpen(false)}>关闭</Button></DialogContent></Dialog>
  </PageFrame>;
}
