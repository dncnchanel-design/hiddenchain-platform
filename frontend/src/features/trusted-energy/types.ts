...
export const trustedViewRoutes: Array<{ key: TrustedViewKey; path: string; label: string; group: string }> = [
  ...
  { key: "asset", path: `${TRUSTED_BASE}/assets/asset-power-output-001`, label: "资产护照", group: "数据空间" },
  { key: "apply", path: `${TRUSTED_BASE}/apply/asset-power-output-001`, label: "使用申请", group: "数据空间" },
  { key: "contract", path: `${TRUSTED_BASE}/contracts/con-202605-001`, label: "合同协商", group: "协作与计算" },
  { key: "ttc", path: `${TRUSTED_BASE}/ttc/ttc-20260518-001`, label: "TTC 任务", group: "协作与计算" },
  { key: "mpc", path: `${TRUSTED_BASE}/mpc/com-20260518-001`, label: "MPC 计算", group: "协作与计算" },
  { key: "results", path: `${TRUSTED_BASE}/results/res-20260518-001`, label: "结果与存证", group: "证据与审计" },
  { key: "audit", path: `${TRUSTED_BASE}/audit`, label: "审计中心", group: "证据与审计" },
];

export const demoAssets: AssetRecord[] = [
  {
    id: "asset-power-output-001",
    name: "新能源出力数据",
    type: "电力数据",
    domain: "发电侧",
    sensitivity: "L4",
    provider: "东部绿能企业",
    providerDid: "did:energy:generator001",
    version: "V3.2",
    ...
    access: "MPC / 聚合",
    format: "JSON / CSV",
    description: "风光场站出力、预测曲线与结算时段汇总，面向受控计算开放聚合指标。",
    status: "可申请",
  },
  {
    id: "asset-load-profile-002",
    name: "组织负荷数据",
    type: "电力数据",
    domain: "用电侧",
    sensitivity: "L3",
    provider: "数联供电服务",
    providerDid: "did:energy:retailer001",
    version: "V2.1",
    ...
    access: "脱敏查询",
    format: "Parquet / CSV",
    description: "聚合到交易单元的负荷曲线，不暴露单一用户原始用电明细。",
    status: "已授权",
  },
  {
    id: "asset-metering-003",
    name: "区域电价核验数据",
    type: "交易数据",
    domain: "市场交易",
    sensitivity: "L2",
    provider: "东部电力交易中心",
    providerDid: "did:energy:trading001",
    version: "V1.3",
    updatedAt: "2026-05-16 08:15",
    quality: 99.1,
    frequency: "日终/次",
    access: "规则匹配",
    format: "JSON",
    description: "交易时段价格、节点归属及规则版本引用，用于结果口径复核。",
    status: "可申请",
  },
  {
    id: "asset-carbon-factor-004",
    name: "区域碳排因子",
    type: "核算数据",
    domain: "绿色核算",
    sensitivity: "L2",
    provider: "算服联合实验室",
    providerDid: "did:energy:carbon-lab001",
    version: "V1.0",
    updatedAt: "2026-05-15 18:32",
    quality: 97.6,
    frequency: "月度/次",
    access: "只读引用",
    format: "JSON / XLSX",
    description: "区域碳排放因子及来源文件摘要，支持绿色电力核验。",
    status: "待复核",
  },
];
...
export const identityProfile = {
  subject: "东部绿能企业",
  role: "数据提供方",
  organizationId: "org-generator001",
  did: "did:energy:generator001",
  issuedAt: "2026-03-01 10:00:00",
  expiresAt: "2026-03-01 10:00:00",
  certificate: "EnergyCA / 2026-03-01",
  connector: "provider-001",
  connectorType: "Provider Connector",
};
...
export const navItems = [
  { key: "workbench" as const, label: "工作台", icon: "LayoutDashboard" },
  { key: "identity" as const, label: "身份中心", icon: "Fingerprint" },
  { key: "catalog" as const, label: "数据目录", icon: "Database" },
  { key: "contract" as const, label: "合同协商", icon: "FileSignature" },
  { key: "mpc" as const, label: "隐私计算", icon: "Network" },
  { key: "results" as const, label: "结果与存证", icon: "BadgeCheck" },
  { key: "audit" as const, label: "审计中心", icon: "ScanSearch" },
];

export function routeForView(key: TrustedViewKey, id?: string) {
  if (key === "asset") return `${TRUSTED_BASE}/assets/${id ?? demoAssets[0].id}`;
  if (key === "apply") return `${TRUSTED_BASE}/apply/${id ?? demoAssets[0].id}`;
  if (key === "contract") return `${TRUSTED_BASE}/contracts/${id ?? "con-202605-001"}`;
  if (key === "ttc") return `${TRUSTED_BASE}/ttc/${id ?? demoTasks[0].id.toLowerCase()}`;
  if (key === "mpc") return `${TRUSTED_BASE}/mpc/${id ?? "com-20260518-001"}`;
  if (key === "results") return `${TRUSTED_BASE}/results/${id ?? "res-20260518-001"}`;
